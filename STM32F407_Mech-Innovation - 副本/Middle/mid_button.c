...
// 配置你按键的返回值
static uint8_t button_one_read(void)     { return key_scan().one;     }
static uint8_t button_two_read(void)   { return key_scan().two;   }
static uint8_t button_three_read(void)  { return key_scan().three;  }
static uint8_t button_four_read(void)   { return key_scan().four;   }
...
void user_button_init(void)
{
    ...
    user_button[BUTTON_ONE].usr_button_read = button_one_read;//按键读值回调函数
    user_button[BUTTON_ONE].cb = (flex_button_response_callback)btn_one_cb;//按键事件回调函数

    user_button[BUTTON_TWO].usr_button_read = button_two_read;//按键读值回调函数
    user_button[BUTTON_TWO].cb = (flex_button_response_callback)btn_two_cb;//按键事件回调函数

    user_button[BUTTON_THREE].usr_button_read = button_three_read;//按键读值回调函数
    user_button[BUTTON_THREE].cb = (flex_button_response_callback)btn_three_cb;//按键事件回调函数

    user_button[BUTTON_FOUR].usr_button_read = button_four_read;//按键读值回调函数
    user_button[BUTTON_FOUR].cb = (flex_button_response_callback)btn_four_cb;//按键事件回调函数
